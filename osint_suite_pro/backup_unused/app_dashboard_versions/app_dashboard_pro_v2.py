# app_dashboard_pro_v3.py — versione PRO completa
# OSINT Intelligence Dashboard v3 (Dark | Zoom + Drag + Heatmap/Clustering | Snapshot HTML)
...
# =============================================================================
ACCENT = "#007BFF"
st.set_page_config(page_title="OSINT Intelligence Dashboard v3", page_icon="🛰️", layout="wide")
init_db()
...
CAPITALS: List[Tuple[str, str, float, float]] = [
    # Lista estesa (puoi sostituire con la lista completa 195 come nel wallboard)
    ...
    ("Lesotho","Maseru",-29.3167,27.4833), ("Morocco","Rabat",34.0209,-6.8416),
]
...
if menu == "Dashboard":
    ...
    if use_heatmap and not points_df.empty:
        ...
        # ✅ Zoom con rotellina + drag fluido + toolbar
    ...
    # Config interattiva valida sia per heatmap che markers
    config = {
        "scrollZoom": True,
        "displayModeBar": True,
    ...
    avg_found_pct = round(df["found_pct_calc"].mean(), 1)

    mc1, mc2, mc3 = st.columns(3)
    mc1.markdown(f"<div class='metric-card'><h3>{total_scans}</h3><p>Scansioni totali</p></div>", unsafe_allow_html=True)
    ...
    with c1:
        ...
        gauge.update_layout(height=260, template="plotly_dark", margin=dict(l=8, r=8, t=30, b=8))
        st.plotly_chart(gauge, use_container_width=True)

    # Donut Piattaforme
    with c2:
        st.markdown("#### 🧭 Piattaforme attive")
        platforms = {}
        for r in df["result"]:
        ...
        if platforms:
            pf_df = pd.DataFrame(platforms.items(), columns=["Piattaforma", "Conteggio"])
            fig_pf = px.pie(pf_df, values="Conteggio", names="Piattaforma", hole=0.55,
                            color_discrete_sequence=px.colors.sequential.Blues)
            fig_pf.update_layout(template="plotly_dark", height=260)
            st.plotly_chart(fig_pf, use_container_width=True)
        else:
            st.info("Nessuna piattaforma attiva nei dati.")

    # Donut Categorie
    with c3:
        st.markdown("#### 🎯 Categorie OSINT")
        categories = {}
        for r in df["result"]:
            for c in r.get("categories", []):
                categories[c] = categories.get(c, 0) + 1
        if categories:
            cat_df = pd.DataFrame(categories.items(), columns=["Categoria", "Conteggio"])
            fig_cat = px.pie(cat_df, values="Conteggio", names="Categoria", hole=0.55,
                             color_discrete_sequence=px.colors.sequential.Blues)
            fig_cat.update_layout(template="plotly_dark", height=260)
            st.plotly_chart(fig_cat, use_container_width=True)
        else:
            st.info("Nessuna categoria disponibile.")

    # =======================
    # ⚡ QUICK SCAN (con auto-save report)
    # =======================
    with st.expander("⚡ Esegui una nuova scansione OSINT", expanded=False):
        col_in1, col_in2, col_in3 = st.columns([3, 1, 1])
        with col_in1:
            quick_user = st.text_input("Username o email:", key="dash_quser_v3")
        with col_in2:
            do_preview = st.checkbox("Preview", value=True, key="dash_prev_v3")
        with col_in3:
            do_github = st.checkbox("GitHub", value=True, key="dash_gh_v3")
        do_status = st.checkbox("Controllo stato profili", value=True, key="dash_status_v3")
        max_profiles = st.slider("Max profili simultanei", 1, 30, 8, key="dash_max_v3")
        auto_report = st.checkbox("Auto-salva PDF/Excel dopo la scansione", value=True, key="dash_autoreport_v3")

        if st.button("🚀 Avvia Scansione Rapida", key="dash_run_v3"):
                ...
                with st.spinner("Esecuzione scansione in corso..."):
                    ...
                    if ps:
                        ...
                        df_profiles = pd.DataFrame([
                            {"Piattaforma": p, "URL": v.get("url",""), "Profilo trovato": "✅" if v.get("exists") else "❌"}
                            for p, v in ps.items()
                        ])
    ...
    if auto_refresh:
        ...
        import time as _t; _t.sleep(refresh_interval); st.rerun()
    ...
    with f1:
        search = st.text_input("Cerca username (parziale o completo)")
    with f2:
        min_d, max_d = df["queried_at"].min().date(), df["queried_at"].max().date()
        date_range = st.date_input("Intervallo date", value=(min_d, max_d))
    ...
    df_f = df[mask]
    st.info(f"Risultati nel filtro: {len(df_f)} scansioni")

    # Timeline
    tl = df_f.groupby(df_f["queried_at"].dt.date).size().reset_index(name="count")
    ...
    if export_snap:
        ...
        figs.append(fig_tl); titles.append("Timeline scansioni")
...
st.caption("© 2025 OSINT Suite Pro — Intelligence Dashboard v3")
