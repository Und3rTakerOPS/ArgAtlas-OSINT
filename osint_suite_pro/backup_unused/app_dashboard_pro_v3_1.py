# app_dashboard_pro_v3_1.py — OSINT Intelligence Dashboard v3.1
# Dark theme | Smart Cache | Combined Filters | Map Zoom/Drag | Heatmap/Clustering | Quick Scan | Entity Graph | Reports/Batch | Snapshot HTML
...
# =============================================================================
ACCENT = "#007BFF"
PAGE_TITLE = "OSINT Intelligence Dashboard v3.1"
...
# HELPERS / UTILITIES
# =============================================================================
# (Lista capitali: versione estesa e ragionevole; puoi sostituire con lista ONU completa se vuoi)
...
if menu == "Dashboard":
    ...
    # Costruisci punti (usiamo deterministic fallback se mancano geo)
    points = []
    ...
    if use_heatmap and not points_df.empty:
        ...
        fig_map = go.Figure()
        # Capitali (layer opzionale)
        if show_capitals:
            cap_layer = pd.DataFrame(cap_df)  # copy
    ...
    # =======================
    # ⚡ QUICK SCAN (auto-save report + smart cache invalidation)
    # =======================
    with st.expander("⚡ Esegui una nuova scansione OSINT", expanded=False):
        col_in1, col_in2, col_in3 = st.columns([3, 1, 1])
        with col_in1:
            quick_user = st.text_input("Username o email:", key="dash_quser_v31")
        with col_in2:
            do_preview = st.checkbox("Preview", value=True, key="dash_prev_v31")
        with col_in3:
            do_github = st.checkbox("GitHub", value=True, key="dash_gh_v31")
        do_status = st.checkbox("Controllo stato profili", value=True, key="dash_status_v31")
        max_profiles = st.slider("Max profili simultanei", 1, 30, 8, key="dash_max_v31")
        auto_report = st.checkbox("Auto-salva PDF/Excel dopo la scansione", value=True, key="dash_autoreport_v31")

        if st.button("🚀 Avvia Scansione Rapida", key="dash_run_v31"):
    ...
    # =======================
    # 🔄 LIVE MONITOR + AUTO-REFRESH (non bloccante)
    # =======================
    st.markdown("---")
    ...
    # =======================
    # 📈 TIMELINE + TAB DETTAGLIO + DONUT (filtro)
    # =======================
    st.markdown("---")
...
st.caption("© 2025 OSINT Suite Pro — Intelligence Dashboard v3.1")
